...
SSN_Z = pd.read_csv('SSN_Z.csv', index_col=0, header=0)
SSN_Z_all = SSN_Z
...
for i in range(result[:, 0].size):
    ax.text(pc_df.iloc[i, 0]+0.05, pc_df.iloc[i, 1]+0.02, pc_df.iloc[i, 2], pc_df.index[i][4], size=10)     # 在每个点边上绘制数据名称

# 设置X、Y、Z轴的名字显示，用刺眼的红色
ax.set_xlabel('PC1', fontdict={'size': 15, 'color': 'black'})
ax.set_ylabel('PC2', fontdict={'size': 15, 'color': 'black'})
ax.set_zlabel('PC3', fontdict={'size': 15, 'color': 'black'})
ax.legend()
handles, labels = plt.gca().get_legend_handles_labels()
by_label = OrderedDict(zip(labels, handles))
plt.legend(by_label.values(), by_label.keys())
# plt.title('PCA visualization of SSN edge statistics')
plt.show()
...
for i in range(12):
    ax.text(pc_df.iloc[i, 0]+0.01, pc_df.iloc[i, 1], pc_df.iloc[i, 2], pc_df.index[i][4], size=15)     # 在每个点边上绘制数据名称
ax.set_xticks([])
ax.set_yticks([])
ax.set_zticks([])
