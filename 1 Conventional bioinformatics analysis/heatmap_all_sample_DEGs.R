...
library(pheatmap)
# 设置工作目录
setwd("E:\\联合利华\\L-DNB_3Dmodel_data\\L-DNB for UVB_SBT ctrl-ref\\PCA visualization based on DNB\\heatmap_DEGs")

# 整理数据集====================================
# 载入数据
dataset1 <- read.table('DEG_CTRL_UVB_data.csv',header = TRUE, row.names = 1, sep = ',')
# 截取表达矩阵的一部分数据来绘制热图
exp_ds = dataset1
...
pheatmap(exp_ds, #表达数据
         ...
         main = 'Heatmap of CTRL and UVB samples based on DEGs'
